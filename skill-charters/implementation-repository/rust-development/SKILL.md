...
Local ESS and Exocore mappings are proposed
integration guidance with their own source owners.
...
## 20. ESS syntax and processing stages

Treat ESS syntax design as source-governed work.
Read the exact source revision defining the relevant vocabulary.
Keep illustrative grammar proposals labeled as proposals.
The package's presentation schema
does not implement a general ESS parser.
Define lexical rules, delimiters, escaping,
ordering, and version behavior before coding.
Preserve original bytes and source spans.
Bound input size, nesting, and expansion.
Reject duplicate singleton fields
and trailing unsupported tokens.
Separate parsed AST, resolved names,
elaborated data, checked IR, and rendered output.
Give failures a stage and stable diagnostic code.
Bind name resolution to an explicit environment revision.
Define normalization losses and test idempotence.
Do not infer semantic preservation from stable output bytes.
Keep parsing pure and source lookup separately controlled.
Do not execute a locator merely
because it appears in source text.
Use references 0011 and 0012 for grammar and stage tests.
Claim only the stages and forms
actually implemented and exercised.
...
## 22. TCCP, EKRP, MINC, and Sigil adaptations

Use the direct framework source for meanings and vocabulary.
Preserve literal labels before normalization.
Carry source identity, revision,
adapter version, and declared losses.
Do not equate similarly named concepts
across frameworks without a sourced mapping.
Keep TCCP ordering and identity
explicit in the chosen representation.
Keep EKRP owner, artifact class, and review state intact.
Do not default missing review information to approved.
Use MINC notation only when it
improves an actual reader decision.
Provide glossary, prose bridge,
assumptions, and non-implications.
A formula field remains presentation text
unless a parser and checker actually process it.
Give Sigil symbols stable IDs, vocabulary
revisions, labels, and readable fallbacks.
Do not use display glyphs as authorization keys.
Escape untrusted text at rendering boundaries.
Test source mismatch, unknown
vocabulary, and omitted distinctions.
Keep each framework's semantic
owner separate from the Rust adapter.
Use references 0014 through 0017 for focused procedures.
Return partial coverage honestly when the
representation supports only selected forms.

## 23. Exocore implementation boundary

Inspect Exocore's current accepted contract
and code before applying architecture guidance.
Let Rust own the domain and lifecycle
responsibilities assigned by that contract.
Keep TypeScript presentation and
clients behind explicit transport contracts.
Use Python capabilities only through a
defined bounded protocol and supervision path.
Hermes remains an independent system.
Do not infer that a Hermes installation
target makes Hermes an Exocore backend.
Keep presentation DTOs separate from executable commands.
Validate untrusted input in Rust
even when the UI already checks it.
Use stable diagnostics across adapters.
Bind concurrent operations to
request, revision, and attempt identity.
Persist before claiming a durable transition.
Record unknown outcomes after lost external replies.
Reconcile before redispatching
work whose completion is uncertain.
Test the actual command adapter and the pure validator.
Historical Exocore test counts remain
historical until rerun on current source.
Use reference 0018 for integration and failure cases.
Do not claim a complete ESS runtime
from a bounded presentation slice.
...
exactly 32 focused supporting notes.
...
Keep the atlas and Ember Circuit
projection synchronized with current source.
...
- [ESS language syntax and grammar](references/0011-ess-language-syntax-and-grammar.md)
- [Elaboration, typechecking, and normalization](references/0012-elaboration-typechecking-and-normalization.md)
- [Proof certificates and Lean bridge](references/0013-proof-certificates-and-lean-bridge.md)
- [TCCP composition and compiler seam](references/0014-tccp-composition-and-compiler-seam.md)
- [EKRP representation and provenance seam](references/0015-ekrp-representation-and-provenance-seam.md)
- [MINC representation choice and prose bridge](references/0016-minc-representation-choice-and-prose-bridge.md)
- [Sigil symbolic layer and readable fallback](references/0017-sigil-symbolic-layer-and-readable-fallback.md)
- [Exocore runtime integration](references/0018-exocore-runtime-integration.md)
